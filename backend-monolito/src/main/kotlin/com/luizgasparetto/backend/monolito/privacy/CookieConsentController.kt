...
class CookieConsentController(
        ...
        private val props: PrivacyProps,
) {
        ...
        private fun hmac(value: String): String {
                ...
                mac.init(
                        SecretKeySpec(
                                props.cookieSignSecret.toByteArray(StandardCharsets.UTF_8),
                                "HmacSHA256"
                        )
                )
                return mac.doFinal(value.toByteArray(StandardCharsets.UTF_8)).joinToString("") {
                        "%02x".format(it)
                }
        }
        ...
        fun saveConsent(
                ...
                val ip =
                        (req.getHeader("X-Forwarded-For") ?: req.remoteAddr ?: "0.0.0.0")
                                .split(",")
                                .first()
                                .trim()
            ...
            """.trimIndent(),
                        mapOf("ip_hash" to ipHash, "ua" to ua, "prefs" to json, "src" to "web")
                )
        }
        ...
        fun getConsent(req: HttpServletRequest): ConsentView? {
                val prefsB64 =
                        req.cookies?.firstOrNull { it.name == "cc_prefs" }?.value ?: return null
